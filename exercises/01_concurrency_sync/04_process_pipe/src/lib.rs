...
pub fn run_command(program: &str, args: &[&str]) -> String {
    // DONE: Use Command::new to create process
    let output = Command::new(program).args(args)
    .stdout(Stdio::piped()).output().unwrap();
    // DONE: Convert stdout to String and return
...
pub fn get_exit_code(command: &str) -> i32 {
    ...
    status.code().unwrap_or(-1) // Return exit code or default (1) if not available
}
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    let mut stdin = child.stdin.take().unwrap();
    stdin.write_all(input.as_bytes()).unwrap();
    drop(stdin); // Close write end to signal EOF
    let mut output = String::new();
    child.stdout.take().unwrap().read_to_string(&mut output).unwrap();
    child.wait().unwrap(); // Wait for grep to exit
    output 
}
